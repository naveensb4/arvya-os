...
import { exchangeGmailCode, markGmailConnected, parseGmailOAuthState } from "@/lib/connectors/gmail";
...
export async function GET(request: Request) {
  ...
  if (!code || !state) return NextResponse.json({ error: "Gmail OAuth callback requires code and state" }, { status: 400 });

  const { brainId, connectorConfigId } = parseGmailOAuthState(state);
  ...
  if (!config || config.connectorType !== "gmail") {
    return NextResponse.json({ error: "Gmail connector config was not found" }, { status: 404 });
  }

  const credentials = await exchangeGmailCode(code, config.credentials ?? undefined);
  await markGmailConnected(config.id, credentials);
