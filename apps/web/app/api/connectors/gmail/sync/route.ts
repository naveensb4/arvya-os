...
export async function POST(request: Request) {
  ...
  if (!connectorConfigId) {
    connectorConfigId = (await getRepository().listConnectorConfigs(selectedBrainId)).find((item) => item.connectorType === "gmail")?.id ?? "";
  }
  if (!connectorConfigId) return NextResponse.json({ error: "Gmail connector config was not found" }, { status: 404 });
