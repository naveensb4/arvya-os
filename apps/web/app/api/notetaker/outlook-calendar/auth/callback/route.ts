...
export async function GET(request: Request) {
  ...
  if (!code || !state) return NextResponse.json({ error: "Outlook Calendar OAuth callback requires code and state" }, { status: 400 });
  ...
  if (!calendar || calendar.provider !== "outlook_calendar") {
    return NextResponse.json({ error: "Outlook Calendar Notetaker calendar was not found" }, { status: 404 });
  }
  ...
  await connectNotetakerCalendar({ calendar, credentials, externalCalendarId: calendar.externalCalendarId });
